...
if __name__ == "__main__":
    ...
    while currDay <= end:
        ...
        wordarray = [np.zeros(numw) for i in range(24)]
        daywordarray = np.zeros(numw)

        print("reading word-vectors/" + lang + "/{0}".format(currDay.strftime("%Y-%m-%d-sum.csv")))
        try:
            f = codecs.open(
                "word-vectors/" + lang + "/{0}-sum.csv".format(currDay.strftime("%Y-%m-%d")),
                "r",
                "utf8",
            )
            ...
            f = codecs.open(
                "word-vectors/{1}/{0}-sumhapps.csv".format(currDay.strftime("%Y-%m-%d"), lang),
                "w",
                "utf8",
            )
