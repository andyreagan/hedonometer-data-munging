...
from numpy import zeros, array, genfromtxt, float
...
def rsync(region,date):
    ...
    if isdir('/usr/share/nginx/data/word-vectors/{0}'.format(region["title"].lower())):
        # try to get the file
        subprocess.call("rsync -avzr --ignore-existing vacc1:/users/a/r/areagan/fun/twitter/jake/pullTweets/word-vectors/{1}-{0}-word-vector.csv /usr/share/nginx/data/word-vectors/{2}/{1}-sum.csv".format(region["title"],datetime.datetime.strftime(date,'%Y-%m-%d'),region["title"].lower()),shell=True)
    else:
        mkdir('/usr/share/nginx/data/word-vectors/{0}'.format(region["title"].lower()))
        subprocess.call("rsync -avzr --ignore-existing vacc1:/users/a/r/areagan/fun/twitter/jake/pullTweets/word-vectors/{1}-{0}-word-vector.csv /usr/share/nginx/data/word-vectors/{2}/{1}-sum.csv".format(region["title"],datetime.datetime.strftime(date,'%Y-%m-%d'),region["title"].lower()),shell=True).communicate()
...
def sumfiles(start,end,wordvec,title,numw):
    ...
    while curr <= end:
        ...
        try:
            ...
            a = genfromtxt('word-vectors/{1}/{0}-sum.csv'.format(curr.strftime('%Y-%m-%d'),title),dtype=float)
            wordvec=wordvec+a
        except:
            # print "could not load {0}".format(curr.strftime('%Y-%m-%d'))
            print 'could not load word-vectors/{1}/{0}-sum.csv'.format(curr.strftime('%Y-%m-%d'),title)
            # raise
            # pass
...
def rest(option,start,end,region,outfile='test.csv',days=[]):
    ...
    if option == 'prevvectors':
        ...
        while maincurr<end:
            ...
            # try:
            [total,date] = sumfiles(maincurr+datetime.timedelta(days=-6),maincurr+datetime.timedelta(days=0),wordvec,region["title"].lower(),numw)
            ...
            f = open('word-vectors/{1}/{0}-prev7.csv'.format(date.strftime('%Y-%m-%d'),region["title"].lower()),'w')
    ...
    if option == 'range':
        ...
        [total,date] = sumfiles(start,end,wordvec,region["title"].lower(),numw)
    ...
    if option == 'list':
        ...
        for day in days:
            ...
            [tmp,date] = sumfiles(day,day,wordvec,region["title"].lower(),numw)
            wordvec+=tmp
...
def timeseries(start,region,useStopWindow=True):
    ...
    g = codecs.open('word-vectors/'+region["title"].lower()+'/sumhapps.csv','a','utf8')
    h = codecs.open('word-vectors/'+region["title"].lower()+'/sumfreq.csv','a','utf8')
    ...
    daywordarray = genfromtxt('word-vectors/'+region["title"].lower()+'/{0}-sum.csv'.format(currDay.strftime('%Y-%m-%d')),dtype=float)
    ...
    if dayhappsarray[0] > 0:
        ...
        f = codecs.open('word-vectors/{1}/{0}-happs.csv'.format(currDay.strftime('%Y-%m-%d'),region["title"].lower()),'w','utf8')
...
def preshift(start,region):
    ...
    word_array = genfromtxt('word-vectors/{1}/{0}-sum.csv'.format(currDay.strftime('%Y-%m-%d'),region["title"].lower()))
    ...
    [sortedMag,sortedWords,sortedType,sumTypes] = shift(previous_word_array_stopped,word_array_stopped,labMTvector,labMTwordList)
    # print sortedMag[:10]
    ...
    g = codecs.open('shifts/{1}/{0}-shift.csv'.format(currDay.strftime('%Y-%m-%d'),region["title"].lower()),'w','utf8')
    ...
    g = open('shifts/{1}/{0}-metashift.csv'.format(currDay.strftime('%Y-%m-%d'),region["title"].lower()),'w')
...
def makeshiftdirectories():
    for region in regions:
        mkdir('/usr/share/nginx/data/shifts/{0}'.format(region["title"].lower()))

def emptysumhapps():
    for region in regions:
        f = open('/usr/share/nginx/data/word-vectors/{0}/sumhapps.csv'.format(region["title"].lower()),'w')
        f.write('date,value\n')
        f.close()
        f = open('/usr/share/nginx/data/word-vectors/{0}/sumfreq.csv'.format(region["title"].lower()),'w')
        f.write('date,value\n')
        f.close()

def sort_sum_happs():
    for region in regions:
        f = open('/usr/share/nginx/data/word-vectors/{0}/sumhapps.csv'.format(region["title"].lower()),'r')
        # skip the header
        f.readline()
        dates = []
        happss = []
        ...
        f = open('/usr/share/nginx/data/word-vectors/{0}/sumfreq.csv'.format(region["title"].lower()),'r')
        # skip the header
        f.readline()
        dates = []
        freqs = []
