...
from labMTsimple import emotion, emotionV, stopper
...
def precomputeTimeseries(fullVec, labMT, labMTvector, labMTwordList, outfile):
    minWindows = 10
...
def renameThe(folder):
    ...
    for movie in query:
        ...
        if not correctname == wrongname:
            try:
                shutil.move(
                    "/usr/share/nginx/data/moviedata/" + folder + "/" + wrongname + ".html",
                    "/usr/share/nginx/data/moviedata/" + folder + "/" + correctname + ".html",
                )
            except:
...
def renameFull(folder):
    ...
    for line in f:
        ...
        print(messytitle)
        shutil.move(
            "/usr/share/nginx/data/moviedata/" + folder + "/" + messytitle,
            "/usr/share/nginx/data/moviedata/" + folder + "/" + cleantitle,
        )
    f.close()
    g.close()
...
def checkClean(folder):
    ...
    for line in f:
        ...
        fsize = os.stat(
            "/usr/share/nginx/data/moviedata/" + folder + "/" + cleantitle + ".end.beg"
        ).st_size
        if fsize < 4000:
            print(messytitle)
...
def process():
    ...
    for movie in query:
        ...
        if os.path.isfile(
            "/usr/share/nginx/data/moviedata/scriptsClean/"
            + titleraw.replace(" ", "-").replace(".", "-")
            + ".txt"
        ):
            ...
            f = codecs.open(
                "scriptsClean/" + titleraw.replace(" ", "-").replace(".", "-") + ".txt", "r", "utf8"
            )
            ...
            words = [
                x.lower()
                for x in re.findall(
                    r"[\w\@\#\'\&\]\*\-\/\[\=\;]+", raw_text_clean, flags=re.UNICODE
                )
            ]
            ...
            for window in windowSizes:
                ...
                chopper(
                    ...
                    labMT,
                    labMTvector,
                    "word-vectors/" + str(window) + "/" + movie.filename + ".csv",
                    minSize=window / 10,
                )
                ...
                if len(fullVec) > 0:
                    if len(fullVec[0]) > 9:
                        precomputeTimeseries(
                            ...
                            labMTvector,
                            labMTwordList,
                            "timeseries/" + str(window) + "/" + movie.filename + ".csv",
                        )
                else:
...
def process_overallHapps():
    ...
    for movie in query:
        ...
        for i in range(len(lines)):
            if lines[i][0:3] != "<b>":
                tmpwords = [
                    x.lower()
                    for x in re.findall(r"[\w\@\#\'\&\]\*\-\/\[\=\;]+", lines[i], flags=re.UNICODE)
                ]
                kwords.extend(tmpwords)
                klines.extend([i for j in range(len(tmpwords))])
    ...
    for k in range(1, len(alltext_labMT_fVec)):
...
def testRE():
    ...
    for movie in query:
        ...
        if os.path.isfile("/usr/share/nginx/data/moviedata/scriptsClean/" + filename + ".txt"):
            ...
            words = [
                x.lower()
                for x in re.findall(r"[\w\@\#\'\&\]\*\-\/\[\=\;]+", raw_text, flags=re.UNICODE)
            ]
