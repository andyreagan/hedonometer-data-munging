...
def sumfiles(start, end, array, lang, numw):
    ...
    while curr <= end:
        ...
        try:
            f = open(
                'word-vectors/{1}/{0}-sum.csv'.format(curr.strftime('%Y-%m-%d'), lang), 'r')
            tmp = f.read().split('\n')
            f.close()
            while len(tmp) > numw:
                del(tmp[-1])
...
if __name__ == '__main__':
    if sys.argv[1] == 'prevvectors':
        ...
        while maincurr < end:
            ...
            # try:
            [total, date] = sumfiles(maincurr+datetime.timedelta(days=-6),
                                     maincurr+datetime.timedelta(days=0), array, lang, numw)

            # write the total into a file for date
            print "printing to {0}-prev7.csv".format(date.strftime('%Y-%m-%d'))
            f = open(
                'word-vectors/{1}/{0}-prev7.csv'.format(date.strftime('%Y-%m-%d'), lang), 'w')
    ...
    if sys.argv[1] == 'sumday':
        ...
        end = datetime.datetime(year, month, day, 0, 0) + \
        ...
        while date < end:
            if not isfile(date.strftime('word-vectors/%Y-%m-%d/%Y-%m-%d-%H-%M.csv')):
                print('missing {0}'.format(date.strftime(
                    'word-vectors/%Y-%m-%d/%Y-%m-%d-%H-%M.csv')))
                allfiles = False
                break
        ...
        if allfiles:
            ...
            date = start
            while date < end:
                a = numpy.genfromtxt(date.strftime(
                    'word-vectors/%Y-%m-%d/%Y-%m-%d-%H-%M.csv'), dtype=numpy.float, delimiter=',')
                print(a)
                my_array += a
                date += fifteen_minutes
