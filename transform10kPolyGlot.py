...
def writeindexfile(name, lang="english"):
    ...
    # slow search
    missed = 0
    index = [0 for x in range(len(words))]
    for i in range(len(words)):
        found = False
        for j in range(len(longerwords)):
...
if __name__ == "__main__":
    ...
    allvecs10k = [[0 for i in range(len(labMTvector))] for j in range(24)]

    print("transforming...")
    for i in range(len(allvecs)):
        for j in range(len(labMTvector)):
            if index[j] > -1:
    ...
    allvecs10ksum = [sum([allvecs10k[j][i] for j in range(24)]) for i in range(len(labMTvector))]
    # i like writing them vertically
    for i in range(len(labMTvector)):
