...
if __name__ == '__main__':
    ...
    f = open(
        'word-vectors/{0}-sumhapps.csv'.format(date.strftime('%Y-%m-%d')), 'r')
    ...
    for i in xrange(7):
        ...
        f = open(
            'word-vectors/{0}-sumhapps.csv'.format(prevdate.strftime('%Y-%m-%d')), 'r')
        tmp = f.readline().rstrip()
        f.close()
    ...
    link = "http://hedonometer.org/index.html?date={0}".format(
        date.strftime('%Y-%m-%d'))
    ...
    tweet = 'why {0} was {2} than the last 7 days: {3}'.format(
        date.strftime('%A, %B %d, %Y'), happs, updown, link)
