...
if __name__ == '__main__':
    ...
    while currDay <= end:
        ...
        print 'reading word-vectors/{1}/{0}'.format(
            currDay.strftime('%Y-%m-%d-sum.csv'), lang)
        try:
            f = codecs.open(
                'word-vectors/{1}/{0}-sum.csv'.format(currDay.strftime('%Y-%m-%d'), lang), 'r', 'utf8')
            wordarray = np.array(map(float, f.read().split('\n')[0:numw]))
            f.close()
            print 'reading word-vectors/{1}/{0}'.format(
                currDay.strftime('%Y-%m-%d-prev7.csv'), lang)
            f = codecs.open(
                'word-vectors/{1}/{0}-prev7.csv'.format(currDay.strftime('%Y-%m-%d'), lang), 'r', 'utf8')
            ...
            wordarrayst = stopper(wordarray, labMTvector, labMTwordList, ignore=[
                                  "lynch", "thirsty", "pakistan", "india", "nigga", "niggas", "niggaz", "nigger"])
            prevwordarrayst = stopper(prevwordarray, labMTvector, labMTwordList, ignore=[
                                      "lynch", "thirsty", "pakistan", "india", "nigga", "niggas", "niggaz", "nigger"])
            ...
            # print sortedType[:10]
            # print sumTypes
            print 'writing shifts/{1}/{0}-shift.csv'.format(
                currDay.strftime('%Y-%m-%d'), lang)
            g = codecs.open(
                'shifts/{1}/{0}-shift.csv'.format(currDay.strftime('%Y-%m-%d'), lang), 'w', 'utf8')
            ...
            g.close()
            print 'writing shifts/{1}/{0}-metashift.csv'.format(
                currDay.strftime('%Y-%m-%d'), lang)
            g = open(
                'shifts/{1}/{0}-metashift.csv'.format(currDay.strftime('%Y-%m-%d'), lang), 'w')
